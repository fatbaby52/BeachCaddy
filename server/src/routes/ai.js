...
router.post('/chat', async (req, res) => {
  try {
    ...
    const systemMessage = {
      role: 'system',
      content: SHOREREADY_CONTEXT + (context ? `\n\nAdditional context:\n${context}` : '')
    }
...
// Beach recommendation endpoint
router.post('/recommend-beach', async (req, res) => {
  try {
    const { groupType, preferences } = req.body

    // Simple rule-based recommendations (fallback if no OpenAI)
    const recommendations = {
      'couple': {
        primary: 'Manresa State Beach',
        reason: 'Quieter and more secluded, perfect for a romantic day together.',
        alternative: 'Capitola Beach - charming village atmosphere with restaurants nearby for dinner after.'
      },
      'family': {
        primary: 'Seacliff State Beach',
        reason: 'Calmer waters for kids, the shipwreck pier is fascinating for all ages, and plenty of space.',
        alternative: 'Rio Del Mar Beach - family-friendly with easy parking and restaurants nearby.'
      },
      'group': {
        primary: 'Sunset State Beach',
        reason: 'Wide sandy beach with plenty of room for large groups, plus bonfire pits for evening gatherings.',
        alternative: 'New Brighton State Beach - beautiful wooded setting, great for nature-loving groups.'
      },
      'solo': {
        primary: 'Manresa State Beach',
        reason: 'Peaceful and less crowded, perfect for relaxation and reflection.',
        alternative: 'New Brighton State Beach - beautiful nature trails nearby if you want to explore.'
      }
    }

    const rec = recommendations[groupType] || recommendations['couple']
    res.json(rec)
